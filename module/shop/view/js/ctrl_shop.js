function ajaxForSearch(url, sData = undefined, total_prod = 0, items_page = 9) {
    localStorage.setItem('page', 1);
    let filters = sData;
    print_buildings(url, { 'filters': filters, 'total_prod': total_prod, 'items_page': items_page });
    load_pagination();

}


function ajaxForSearchScroll(url, sData = undefined, total_prod = 0, items_page = 4) {
        let filters = sData;
        print_scroll(url, { 'filters': filters, 'total_prod': total_prod, 'items_page': items_page });
        load_scroll();
}
...
function print_points() {
    ...
    ajaxPromise('POST', 'JSON', 'index.php?module=shop&op=load_buildings', { 'filters': filters, 'total_prod': 0, 'items_page': 100000 })
        .then(function (data) {
            mapBox_all(data);//
        })
        .catch(function () {
...
function load_pagination() {
    ...
    ajaxPromise("POST", "JSON", 'index.php?module=shop&op=total_prod', { 'filters': filters })
        .then(function (data) {
            ...
            $(".page").click(function () {
                ...
                print_buildings("index.php?module=shop&op=load_buildings", { 'filters': filters, 'total_prod': offset_prod, 'items_page': 9 });
                window.scrollTo(0, 0);
    ...
    ajaxPromise("POST", "JSON", "index.php?module=shop&op=total_prod", { 'filters': filters })
        .then(function (data) {
            ...
            $(".scroll_button").click(function () {
                ...
                print_scroll("index.php?module=shop&op=load_buildings", { 'filters': filters, 'total_prod': offset_prod, 'items_page': 4 });
                if (cliks >= total_cliks - 1) {
                    print_scroll("index.php?module=shop&op=view_refill", { 'filters': filters, 'items_page': extra_buildings });
...
function loadDetails(id_building) {
    ...
    ajaxPromise("POST", "JSON", "index.php?module=shop&op=details_building", { id_building: id_building })
        .then(function (data) {
            ...
            if (lasts_views == null) {
                ...
                ajaxPromise("POST", "JSON", "index.php?module=shop&op=count_click_details", { id: id })
                    .then(function (data) {
            ...
            ajaxForSearchScroll("index.php?module=shop&op=load_buildings", filters);
...
function recharge_filters() {
    ...
    ajaxForSearch("index.php?module=shop&op=load_buildings", filters);
}
...
function click_like_building() {
    $(document).on("click", ".button_like_building_active", function (event) {
        ...
        ajaxPromise("POST", "JSON", "index.php?module=shop&op=action_like", { "id_building": id })
            .then(function (data) {
